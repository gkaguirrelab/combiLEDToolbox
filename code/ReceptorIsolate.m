function [isolatingPrimary,fVal] = ReceptorIsolate(T_receptors,whichReceptorsToIsolate, ...
    whichReceptorsToIgnore,whichReceptorsToMinimize, B_primary,backgroundPrimary,initialPrimary, ...
    whichPrimariesToPin,primaryHeadRoom,~,desiredContrasts,ambientSpd,minimumNonZeroSetting)
...
myObj = @(x) IsolateFunction(x,B_primary,backgroundPrimary,ambientSpd,T_receptors,whichReceptorsToIsolate,desiredContrasts,whichReceptorsToMinimize);
myNonlcon = @(x) enforceSettingThreshold(x,backgroundPrimary,minimumNonZeroSetting);
[x,fVal] = fmincon(myObj,x0,[],[],Aeq,beq,vlb,vub,myNonlcon,options);
...
% This local function enforces a non-linear constraint that requires the
% minimum setting of any modulation to not fall between zero and
% minimumNonZeroSetting
function [c, ceq] = enforceSettingThreshold(x,backgroundPrimary,minimumNonZeroSetting)

closeEnoughToZero = 0.01;
midThresh = minimumNonZeroSetting/2;
c = zeros(size(x));
modPosSettings = x;
badIdx = find(modPosSettings < minimumNonZeroSetting & modPosSettings > closeEnoughToZero);
c(badIdx) = (midThresh-abs(modPosSettings(badIdx)-midThresh)).^2;

modNegSettings = backgroundPrimary-(x-backgroundPrimary);
badIdx = find(modNegSettings < minimumNonZeroSetting & modNegSettings > closeEnoughToZero);
c(badIdx) = (midThresh-abs(modNegSettings(badIdx)-midThresh)).^2;
ceq = [];
